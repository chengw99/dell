# -*- coding: utf-8 -*-
"""
Created on Thu Nov 15 15:07:46 2018
...
# 可与pic-one.py搭配使用，由它获取合适的图像高度值，即 m 的值，不可根据这个程序生成的图片判断顶盖的位置是否为1.2m，其实就是判断取数据的方向和范围，大概是否正确
...
data = pd.read_csv(ipath+'\\'+'one-facing north-00.csv',names=np.arange(480)) # 选择不同时段的红外数据文件
...
plt.pcolormesh(X,Y,a,cmap='jet',vmin=20,vmax=40)
