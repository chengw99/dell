# -*- coding: utf-8 -*-
"""
Created on Fri Nov 23 11:33:14 2018
...
# 可与pic-two.py搭配使用，由它获取合适的图像高度值，即 m 的值，不可根据这个程序生成的图片判断顶盖的位置是否为1.2m，其实就是判断取数据的方向和范围，大概是否正确
...
data = pd.read_csv(ipath+'\\'+'two-facing south-23.csv',names=np.arange(480)) # 选择不同时段的红外数据文件
