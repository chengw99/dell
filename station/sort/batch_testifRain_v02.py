...
for name in l:
    ...
    for i in k:
        ...
        if not qExist:
            ...
            pass
        #复制数据到工作目录
        qfile = os.listdir(spath+'\\'+r'6月份'+'\\'+name+'\\'+r'total data')
        for q in qfile:
            try:
                shutil.copyfile(spath+'\\'+r'6月份'+'\\'+name+'\\'+r'total data'+'\\'+q,
                                ipath+'\\'+name+'\\'+r'total data'+'\\'+q)
            except:
                pass
    mk  = []        
    wfile = glob.glob(ipath+'\\'+name+'\\'+r'total data'+'\\'+'*.csv')
    for w in wfile:
        data = pd.read_csv(w)
        ...
        if len(y) == 144:
            ...
            result.to_csv(ipath+'\\'+name+'\\'+r'rain\data'+'\\'+name+'-'+w[-12:-4]+'-'+'Precipitation.csv',columns=columns)
            #-------------------------判断是否下雨------------------------------#
