# -*- coding: utf-8 -*-
"""
Created on Mon Apr  9 11:33:27 2018
...
for i in h:
    for c in n:
        rExist = os.path.exists(ipath+'\\'+i+'\\'+c)
        if not rExist:
            os.makedirs(ipath+'\\'+i+'\\'+c)
        else:
            pass
    for j in l:
        qExist = os.path.exists(ipath+'\\'+i+'\\'+r'data'+'\\'+j)
        if not qExist:
...
for i in h:
    for j in l:
        ...
        for d in dfile:
            sfile = glob.glob(ipath+'\\'+i+'\\'+r'data'+'\\'+j+'\\'+d+'\\'+'*.csv') #与上个程序不同的是此处
        
            if len(sfile) == 4:
                
                data1 = pd.read_csv(sfile[1],engine = 'python') # 30cm
                data2 = pd.read_csv(sfile[2],engine = 'python') # 60cm
                data3 = pd.read_csv(sfile[3],engine = 'python') # 90cm
                data4 = pd.read_csv(sfile[0],engine = 'python') # 110cm
...
bpath = r'C:\Users\DELL\Desktop\处理数据\picture\thermo\中空-装沙-不同高度'
