        ...
        if file[-3:] == 'csv':
            ...
            for n in num:
                ...
                try:
                    ...
                    wExist = os.path.exists(opath+'\\'+r'data'+'\\'+file[-12:-4]+'\\'+f[-11:]+'\\'+dict[str(n)][-1])
                    if not wExist:
                        os.makedirs(opath+'\\'+r'data'+'\\'+file[-12:-4]+'\\'+f[-11:]+'\\'+dict[str(n)][-1])
                        result.to_csv(opath+'\\'+r'data'+'\\'+file[-12:-4]+'\\'+f[-11:]+'\\'+dict[str(n)][-1]+'\\'+f[0]+dict[str(n)][-1]+'-'+file[-12:],columns = columns)
                    else:
                        result.to_csv(opath+'\\'+r'data'+'\\'+file[-12:-4]+'\\'+f[-11:]+'\\'+dict[str(n)][-1]+'\\'+f[0]+dict[str(n)][-1]+'-'+file[-12:],columns = columns)
...
for w in wfile:
    ...
    for t in tfile:
        bbfile = os.listdir(opath+'\\'+r'data'+'\\'+w+'\\'+t)
        for bb in bbfile:
            u = glob.glob(opath+'\\'+r'data'+'\\'+w+'\\'+t+'\\'+bb+'\\'+'*.csv')
        
            if len(u) == 3:
                if t == 'empty model':
                    data1 = pd.read_csv(u[0]) # B-HW1
                    data2 = pd.read_csv(u[2]) # F-HW2
                    data3 = pd.read_csv(u[1]) # C-HW3
                else:
                    data1 = pd.read_csv(u[2]) # E-HW1
                    data2 = pd.read_csv(u[0]) # A-HW2
                    data3 = pd.read_csv(u[1]) # D-HW3
                ...
                plt.plot(x,y1,linestyle='-',marker='D',markersize=5,markerfacecolor='none',markevery=6,linewidth=0.8,label='HW1'+'-'+w)
                plt.plot(x,y2,linestyle='-',marker='o',markersize=5,markerfacecolor='none',markevery=6,linewidth=0.8,label='HW2'+'-'+w)
                plt.plot(x,y3,linestyle='-',marker='s',markersize=5,markerfacecolor='none',markevery=6,linewidth=0.8,label='HW3'+'-'+w)
                ...
                plt.savefig(opath+'\\'+r'total picture'+'\\'+'thermo-different HW-'+t+'-'+bb+'-'+w+'.png')
                plt.savefig(opath+'\\'+r'data'+'\\'+w+'\\'+t+'\\'+bb+'\\'+'thermo-different HW-'+t+'-'+bb+'-'+w+'.png')
                plt.close()
#--------------------------移动结果到最终文件夹-----------------------------------#                
kpath = r'C:\Users\DELL\Desktop\处理数据\picture\thermo\空气-对比不同高宽比'
