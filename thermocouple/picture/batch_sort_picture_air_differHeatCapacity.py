# -*- coding: utf-8 -*-
"""
Created on Tue Apr 24 15:05:57 2018

@author: DELL
"""

#想想还是顺便把图也画出来好了
#此程序用于分类热电偶空气温度 - 不同建筑热容
        ...
        if file[-3:] == 'csv':
            ...
            for n in num:
                ...
                try:
                    ...
                    wExist = os.path.exists(opath+'\\'+r'data'+'\\'+file[-12:-4]+'\\'+f[2:5]+'\\'+dict[str(n)][-1])
                    if not wExist:
                        os.makedirs(opath+'\\'+r'data'+'\\'+file[-12:-4]+'\\'+f[2:5]+'\\'+dict[str(n)][-1])
                        result.to_csv(opath+'\\'+r'data'+'\\'+file[-12:-4]+'\\'+f[2:5]+'\\'+dict[str(n)][-1]+'\\'+f[0]+dict[str(n)][-1]+'-'+file[-12:],columns = columns)
                    else:
                        result.to_csv(opath+'\\'+r'data'+'\\'+file[-12:-4]+'\\'+f[2:5]+'\\'+dict[str(n)][-1]+'\\'+f[0]+dict[str(n)][-1]+'-'+file[-12:],columns = columns)
    ...
    for t in tfile:
        ...
        for bb in bbfile:
            ...
            if len(u) == 2:
                ...
                if t == 'HW1' or 'HW3':
                    data1 = pd.read_csv(u[0]) # B-empty
                    data2 = pd.read_csv(u[1]) # E-sand
                else:
                    data1 = pd.read_csv(u[1]) # F-empty
                    data2 = pd.read_csv(u[0]) # A-sand
                ...
                plt.plot(x,y1,linestyle='-',marker='D',markersize=7,markerfacecolor='none',markevery=6,linewidth=1,label='empty'+'-'+w)
                plt.plot(x,y2,linestyle='-',marker='o',markersize=7,markerfacecolor='none',markevery=6,linewidth=1,label='sand'+'-'+w)
                ...
                plt.savefig(opath+'\\'+r'total picture'+'\\'+'thermo-different HeatCapacity-'+t+'-'+bb+'-'+w+'.png')
                plt.savefig(opath+'\\'+r'data'+'\\'+w+'\\'+t+'\\'+bb+'\\'+'thermo-different HW-'+t+'-'+bb+'-'+w+'.png')
                plt.close()
#--------------------------移动结果到最终文件夹-----------------------------------#                
kpath = r'C:\Users\DELL\Desktop\处理数据\picture\thermo\空气-对比不同建筑热容'
