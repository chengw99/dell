...
#此程序用于批量作图-热电偶-10min平均的数据-对比不同高度
#输出一个txt,里面列举了空文件夹的个数
#只需要修改切片范围，就可以获取相应大的文件夹
import os
import glob
import shutil
import numpy as np
...
if not tExist:
    ...
    jExist = os.path.exists(r'C:\Users\DELL\Desktop\处理数据\picture\thermo\对比不同高度'+'\\'+f[:24])
    if not jExist:
        os.mkdir(r'C:\Users\DELL\Desktop\处理数据\picture\thermo\对比不同高度'+'\\'+f[:24])
    else:
        pass

    b_file = os.listdir(r'e:\py_data\thermo')
    for b in b_file:
        shutil.move(r'e:\py_data\thermo\\'+b,r'C:\Users\DELL\Desktop\处理数据\picture\thermo\对比不同高度'+'\\'+f[:24]+'\\'+b)
    
    mExist = os.path.exists(r'C:\Users\DELL\Desktop\处理数据\picture\thermo\对比不同高度'+'\\'+f[:24]+'\\'+r'汇总')
    if not mExist:
        os.mkdir(r'C:\Users\DELL\Desktop\处理数据\picture\thermo\对比不同高度'+'\\'+f[:24]+'\\'+r'汇总')
    else:
        pass

    m_file = os.listdir(r'e:\py_output')
    for m in m_file:
        shutil.move(r'e:\py_output\\'+m,r'C:\Users\DELL\Desktop\处理数据\picture\thermo\对比不同高度'+'\\'+f[:24]+'\\'+r'汇总'+'\\'+m)

    dt = '空文件夹总数:'+str(num)
    k.append(dt)
    with open(r'C:\Users\DELL\Desktop\处理数据\picture\thermo\对比不同高度'+'\\'+f[:24]+'\\'+r'empty folder.txt','w') as f:
        for i in k:
            f.write(i)
            f.write('\n')
